          ...
          <div className="flex flex-col gap-10">
            {educationData.map((edu, i) => (
              ...
              >
                ...
                <div className="bg-base-200 rounded p-6 shadow-md hover:shadow-lg transition-all duration-300 flex-1">
                  <h3 className="text-xl font-semibold mb-1">{edu.degree}</h3>
                  <p className="text-base-content/80 font-medium">{edu.institution}</p>
                  {edu.year && (
                    <p className="text-base-content/70 mt-1">
                      <span className="font-medium">Year:</span> {edu.year} | <span className="font-medium">GPA:</span> {edu.gpa}
                    </p>
                  )}
                  {edu.duration && (
                    <p className="text-base-content/70 mt-1">
                      <span className="font-medium">Duration:</span> {edu.duration} | <span className="font-medium">Expected Graduation:</span> {edu.expected}
                    </p>
                  )}
                </div>
              </motion.div>
